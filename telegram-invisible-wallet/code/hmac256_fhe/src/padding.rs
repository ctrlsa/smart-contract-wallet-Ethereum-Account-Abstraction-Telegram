...
pub fn pad_sha256_input(input: &str) -> Vec<bool> {
    ...
    pad_sha256_data(&input_bytes)
}

pub fn compute_padding(length_i1: usize, length_i2: usize) -> Vec<bool> {
    // Append a single '1' bit
    let mut bits = vec![true];

    // Calculate the number of padding zeros required
    let padding_zeros = (512 - ((length_i1 + length_i2 + 1 + 64) % 512)) % 512;
    eprintln!("padding_zeros = {:#?}", padding_zeros);
    ...
    let data_len_bits = (length_i1 as u64) + (length_i2 as u64);
    eprintln!("data_len_bits = {:#?}", data_len_bits);
...
pub fn string_as_bool_vec(input: &str) -> Vec<bool> {
    let input_bytes = input.as_bytes().to_vec();
    input_bytes.iter().flat_map(|byte| (0..8).rev().map(move |i| (byte >> i) & 1 == 1)).collect::<Vec<_>>()
}
